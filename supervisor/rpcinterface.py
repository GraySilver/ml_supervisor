...
class SupervisorNamespaceRPCInterface:
    ...
    def _get_supervisor_process_file(self, config_file):
        conf = configparser.ConfigParser()
        conf.read(config_file)
        return conf.get('include', 'files')

    def get_process_config_file(self):
        process_file_path = self._get_supervisor_process_file(self.supervisord.options.configfile)
        process_file_path = '/'.join(process_file_path.split('/')[:-1])
        if os.path.exists(process_file_path):
            dirs = os.listdir(process_file_path)
            if len(dirs) > 0:
                return [str(i).split('.')[0] for i in dirs]
        else:
            return []

    def _create_process_conf(self, name, cmd, directory=None, stdout_logfile=None, stderr_logfile=None, file_path=None):
        conf = configparser.ConfigParser()
        section_name = 'program:%s' % name
        conf.add_section(section_name)
        conf.set(section_name, 'command', cmd)
        conf.set(section_name, 'startsecs', '5')
        conf.set(section_name, 'stopwaitsecs', '10')
        conf.set(section_name, 'autostart', 'false')
        conf.set(section_name, 'autorestart', 'false')
        conf.set(section_name, 'stopasgroup', 'true')
        conf.set(section_name, 'killasgroup', 'true')
        if directory is not None:
            conf.set(directory, 'directory', directory)
        if stdout_logfile is not None:
            conf.set(section_name, 'stdout_logfile', stdout_logfile)
        if stderr_logfile is not None:
            conf.set(section_name, 'stderr_logfile', stderr_logfile)
        if file_path is None:
            conf.write(open("%s.ini" % name, "w"))
        else:
            conf.write(open(file_path, "w"))

    def auto_config(self, name, cmd):
        process_file_path = self._get_supervisor_process_file(self.supervisord.options.configfile)
        process_file_path = str(process_file_path).replace('*', name)
        log_file_path = self.supervisord.options.logfile

        stdout_logfile = '/'.join(log_file_path.split('/')[:-1]) + '/%s.log'%name
        stderr_logfile = '/'.join(log_file_path.split('/')[:-1]) + '/%s.err'%name
    ...
    def auto_run(self, name, cmd):
        ...
        return self.startProcess(name=name)
